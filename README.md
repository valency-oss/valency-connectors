...
Review Valency Bond tools individually before adding them to an allow list.
Do not allow the entire Claude `mcp__plugin_valency_valency__*` namespace as a
wildcard: it includes the write-capable `submit_feedback` tool, which should
remain subject to explicit authorization.

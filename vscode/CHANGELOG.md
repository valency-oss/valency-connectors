...
- Reinstalling from the Extensions view showed only the sign-in notification, not the walkthrough. The check for an already-open walkthrough could not tell the "Extension: Valency" details tab apart from the walkthrough tab, so it skipped opening. The opener now judges success by what the open actually changed, and logs each attempt to the Valency output channel.

...
test("Kiro documentation uses the supported root GitHub lifecycle", () => {
  ...
  assert.match(
    development,
    /do not\s+prove that Kiro can\s+install the Power,\s+complete OAuth, or invoke a remote tool/,
  );
...
test("Copilot documentation uses marketplace-first CLI-only support", () => {
  ...
  assert.match(development, /Copilot and Cursor marketplaces and provider packages/);
  assert.match(
    development,
...
test("Cursor documentation keeps installation interactive and profile-free", () => {
  ...
  assert.match(
    development,
    /Cursor package checks do not\s+prove\s+that Cursor can\s+install the plugin,\s+complete OAuth, or invoke a remote tool/,
  );
});
